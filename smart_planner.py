...
.toast.meeting-remind{border-color:var(--cyan);color:var(--cyan2);background:var(--cyan-bg)}
...
.notif-banner{position:fixed;top:80px;right:24px;z-index:2500;max-width:380px;padding:16px 20px;border-radius:14px;box-shadow:var(--shadow-lg);animation:slide-notif .4s ease;display:flex;align-items:flex-start;gap:12px}
.notif-banner.meeting-notif{background:var(--cyan-bg);border:1.5px solid var(--cyan);color:var(--cyan2)}
.notif-banner.overdue-notif{background:var(--red-bg);border:1.5px solid var(--red);color:var(--red)}
.notif-banner .notif-icon{font-size:24px;flex-shrink:0}
.notif-banner .notif-body{flex:1}
.notif-banner .notif-title{font-weight:700;font-size:14px;margin-bottom:2px}
.notif-banner .notif-text{font-size:12px;opacity:.85}
.notif-banner .notif-close{background:none;border:none;font-size:18px;color:inherit;opacity:.6;padding:0;margin-left:8px}
.notif-banner .notif-close:hover{opacity:1}
@keyframes slide-notif{from{opacity:0;transform:translateX(100px)}to{opacity:1;transform:translateX(0)}}
...
const cls=notif.type==="meeting"?"meeting-notif":"overdue-notif";
const icon=notif.type==="meeting"?"🕐":"🔴";
return<div className={`notif-banner ${cls}`}>
...
const[f,setF]=useState(initial?{...initial,cat:initial.cat||cats[0]||""}:{name:"",cat:cats[0]||"",pri:pris[1]||pris[0]||"",desc:"",start:td(),deadline:td(),timeStart:"",timeEnd:"",progress:0,notes:"",result:""});
const s=(k,v)=>setF(p=>({...p,[k]:v}));
const go=()=>{if(!f.name.trim())return;onSubmit({...f,progress:parseInt(f.progress)||0,type:"meeting"});onClose()};
return<div className="mo" onClick={onClose}><div className="md" onClick={e=>e.stopPropagation()}>
...
{initial&&<div className="fl"><label style={{color:"var(--green2)"}}>📝 Результаты совещания</label><textarea value={f.result||""} onChange={e=>s("result",e.target.value)} placeholder="Итоги, решения, следующие шаги..." style={{minHeight:80,borderColor:"var(--green)",background:"var(--green-bg)"}}/></div>}
<button className="bp bp-cyan" onClick={go} style={{marginTop:6}}>{initial?"💾 Сохранить":"➕ Добавить совещание"}</button>
</div></div></div></div>}


/* ─── Result Form (for completing meetings) ─── */
function ResultForm({task,onClose,onSave}){
const[result,setResult]=useState(task.result||"");
const go=()=>{onSave(task.id,{result,progress:100,completedAt:td()});onClose()};
return<div className="mo" onClick={onClose}><div className="md" onClick={e=>e.stopPropagation()} style={{width:"min(500px,94vw)"}}>
<div className="mh" style={{background:"var(--green-bg)"}}><h3>📝 Результаты совещания</h3><button className="mc" onClick={onClose}>×</button></div>
<div className="mb"><div style={{display:"flex",flexDirection:"column",gap:14}}>
<div style={{background:"var(--bg)",borderRadius:12,padding:14}}>
<div style={{fontWeight:700,fontSize:15,marginBottom:4}}>{task.name}</div>
<div style={{fontSize:12,color:"var(--text3)"}}>{fmt(task.start)} {task.timeStart&&`· ${task.timeStart}–${task.timeEnd||"?"}`}</div>
</div>
<div className="fl"><label style={{color:"var(--green2)"}}>📝 Итоги, решения, следующие шаги</label>
<textarea value={result} onChange={e=>setResult(e.target.value)} placeholder="Опишите результаты совещания, принятые решения, назначенные ответственные, дальнейшие шаги..." style={{minHeight:140,borderColor:"var(--green)",background:"var(--green-bg)"}} autoFocus/></div>
<button className="bs" onClick={go} style={{width:"100%"}}>✅ Сохранить и завершить совещание</button>
<button className="bo" onClick={()=>{onSave(task.id,{result});onClose()}} style={{width:"100%",textAlign:"center"}}>💾 Сохранить без завершения</button>
</div></div></div></div>}


/* ─── Task/Meeting Detail side panel ─── */
function TDetail({task,onClose,onEdit,onDelete,onProg,onResult,now}){
...
{isM&&task.result&&<div style={{marginBottom:18}}><div style={{fontSize:10,fontWeight:700,color:"var(--green2)",textTransform:"uppercase",marginBottom:6}}>📝 Результаты совещания</div><div style={{background:"var(--green-bg)",borderRadius:12,padding:14,fontSize:14,color:"var(--text)",lineHeight:1.6,border:"1px solid rgba(16,185,129,.3)"}}>{task.result}</div></div>}
{isM&&!task.result&&task.progress<100&&<div style={{marginBottom:18,background:"var(--amber-bg)",borderRadius:12,padding:14,border:"1px solid rgba(245,158,11,.2)"}}><div style={{fontSize:12,color:"var(--amber2)",fontWeight:600}}>⏳ Результаты совещания ещё не внесены</div></div>}
</div>
...
{isM&&<button className="bo" style={{borderColor:"var(--green)",color:"var(--green2)"}} onClick={()=>onResult(task)}>📝 Результаты</button>}
...
const doAddCat=async()=>{if(!nc.trim())return;await callApi("add_category",nc.trim());setNc("");onUpdate()};
...
const notifiedIdsRef=useRef({meeting:new Set(),overdue:new Set()});

const addNotification=(notif)=>{const id=Date.now()+Math.random();setNotifications(prev=>[...prev,{...notif,id}]);setTimeout(()=>setNotifications(prev=>prev.filter(n=>n.id!==id)),8000)};
...
const saveResult=async(id,patch)=>{await callApi("update_task",id,JSON.stringify(patch));reload()};
...
const curDate=timeData.date;
const curTime=timeData.time;
const curMinutes=parseInt(curTime.split(":")[0])*60+parseInt(curTime.split(":")[1]);
...
if(m.start!==curDate&&!(m.start<=curDate&&m.deadline&&m.deadline>=curDate))return;
const[h,mi]=m.timeStart.split(":").map(Number);
const meetMin=h*60+mi;
const diff=meetMin-curMinutes;
if(diff>0&&diff<=5&&!notifiedIdsRef.current.meeting.has(m.id)){
notifiedIdsRef.current.meeting.add(m.id);
addNotification({type:"meeting",title:`🕐 Совещание через ${diff} мин`,text:m.name+(m.timeStart?` · ${m.timeStart}`:"")});
callApi("update_task",m.id,JSON.stringify({notifiedMeeting:1}));
}
});

// Check overdue tasks (one-time notification)
data.tasks.filter(t=>t.type!=="meeting"&&t.name&&t.progress<100).forEach(t=>{
if(!t.deadline)return;
if(t.deadline<curDate&&!notifiedIdsRef.current.overdue.has(t.id)&&!t.notifiedOverdue){
notifiedIdsRef.current.overdue.add(t.id);
addNotification({type:"overdue",title:"🔴 Задача просрочена",text:`«${t.name}» — дедлайн был ${fmt(t.deadline)}`});
...
/* Export meetings for the same period */
const expMeetings=useMemo(()=>meetings.filter(m=>{
if(!m.name)return false;
if(m.start&&m.start>expTo)return false;
if(m.deadline&&m.deadline<expFrom)return false;
return true
}).sort((a,b)=>(a.start||"").localeCompare(b.start||"")||(a.timeStart||"").localeCompare(b.timeStart||"")),[meetings,expFrom,expTo]);
...
/* Meeting row for export */
const mtgRow=(m)=>{const ms=getMS(m,now);const msColor=ms.c==='var(--green)'?'#10B981':ms.c==='var(--cyan2)'?'#0891B2':ms.c==='var(--amber)'?'#F59E0B':'#94A3B8';
return`<tr><td><strong>${esc(m.name)}</strong>${m.desc?'<div style="color:#64748B;font-size:11px;margin-top:2px">'+esc(m.desc)+'</div>':''}</td><td>${fmt(m.start)}</td><td>${m.timeStart?m.timeStart+(m.timeEnd?' – '+m.timeEnd:''):'—'}</td><td><span class="bdg" style="color:${gp(m.pri)};background:${gp(m.pri)}15">${esc(m.pri)}</span></td><td style="color:${msColor};font-weight:600">${ms.l}</td><td>${m.result?'<div style="background:#F0FDF4;border-radius:6px;padding:6px 8px;font-size:11px;border:1px solid #BBF7D0">'+esc(m.result)+'</div>':'<span style="color:#94A3B8">—</span>'}</td></tr>`};

const mtgDone=expMeetings.filter(m=>m.progress>=100||m.completedAt);
const mtgUpcoming=expMeetings.filter(m=>m.progress<100&&(!m.deadline||m.deadline>=now));
const mtgPast=expMeetings.filter(m=>m.progress<100&&m.deadline&&m.deadline<now);
const mtgWithResults=expMeetings.filter(m=>m.result);
const mtgWithoutResults=expMeetings.filter(m=>(m.progress>=100||(m.deadline&&m.deadline<now))&&!m.result);
...
.mtg-header{background:linear-gradient(135deg,rgba(6,182,212,.08),rgba(6,182,212,.02));border:1.5px solid rgba(6,182,212,.3);border-radius:14px;padding:20px 24px;margin-bottom:20px}
.mtg-header h2{color:#0891B2;border-bottom-color:rgba(6,182,212,.3)}
.mtg-kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:20px}
.mtg-kpi{border:1.5px solid rgba(6,182,212,.2);border-radius:10px;padding:12px;text-align:center;background:rgba(6,182,212,.04)}
.mtg-kpi .num{font-size:22px;font-weight:800;color:#0891B2;font-family:'IBM Plex Mono',monospace}
.mtg-kpi .lbl{font-size:9px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:.8px;margin-top:4px}
.result-box{background:#F0FDF4;border:1px solid #BBF7D0;border-radius:10px;padding:14px;margin-top:8px}
.result-box .res-title{font-size:10px;font-weight:700;color:#059669;text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px}
.result-box .res-text{font-size:12px;color:#1E293B;line-height:1.6}
...
<div><h1>⚡ Smart Planner</h1><div class="subtitle">Отчёт о выполнении задач и совещаниях</div></div>
<div class="period">
...
${expStats.total>0?`<div class="section no-break"> <h2>🔀 Структура задач</h2> <div class="bar-section"> <div class="bar-group"><div class="bar-group-title">🏷️ По категориям</div> ${Object.entries(catBr).sort((a,b)=>b[1]-a[1]).map(([c,n])=>`<div class="bar-item"><div class="bar-label-row"><span style="font-weight:600;color:${CC[c]||'#64748B'}">${esc(c)}</span><span style="font-family:IBM Plex Mono;font-weight:700">${n} · ${Math.round(n/expStats.total*100)}%</span></div><div class="bar-track"><div class="bar-inner" style="width:${(n/expStats.total)*100}%;background:${CC[c]||'#64748B'}"></div></div></div>`).join('')} </div> <div class="bar-group"><div class="bar-group-title">🎯 По приоритетам</div> ${Object.entries(priBr).sort((a,b)=>b[1]-a[1]).map(([p,n])=>`<div class="bar-item"><div class="bar-label-row"><span style="font-weight:600;color:${PC[p]||'#64748B'}">${esc(p)}</span><span style="font-family:IBM Plex Mono;font-weight:700">${n} · ${Math.round(n/expStats.total*100)}%</span></div><div class="bar-track"><div class="bar-inner" style="width:${(n/expStats.total)*100}%;background:${PC[p]||'#64748B'}"></div></div></div>`).join('')} </div></div></div>`:''}


${overdue.length?`<div class="section page-break"><h2 style="color:#EF4444">⚠ Просроченные задачи <span class="count">${overdue.length}</span></h2> <table><thead><tr><th>Задача</th><th>Категория</th><th>Приоритет</th><th>Дедлайн</th><th>Статус</th><th>Прогресс</th></tr></thead><tbody>${overdue.map(t=>taskRow(t,true)).join('')}</tbody></table></div>`:''}


${burning.length?`<div class="section"><h2 style="color:#F59E0B">🔥 Горящие задачи <span class="count">${burning.length}</span></h2> <table><thead><tr><th>Задача</th><th>Категория</th><th>Приоритет</th><th>Дедлайн</th><th>Статус</th><th>Прогресс</th></tr></thead><tbody>${burning.map(t=>taskRow(t,true)).join('')}</tbody></table></div>`:''}


${inprog.length?`<div class="section"><h2 style="color:#4361EE">⟳ Задачи в работе <span class="count">${inprog.length}</span></h2> <table><thead><tr><th>Задача</th><th>Категория</th><th>Приоритет</th><th>Дедлайн</th><th>Статус</th><th>Прогресс</th></tr></thead><tbody>${inprog.map(t=>taskRow(t,true)).join('')}</tbody></table></div>`:''}


${done.length?`<div class="section page-break"><h2 style="color:#10B981">✓ Выполненные задачи <span class="count">${done.length}</span></h2> <table><thead><tr><th>Задача</th><th>Категория</th><th>Приоритет</th><th>Статус</th><th>Прогресс</th><th>Завершено</th></tr></thead><tbody>${done.map(t=>taskRow(t,false)).join('')}</tbody></table></div>`:''}


<!-- ═══ MEETINGS SECTION ═══ -->
<div class="page-break"></div>
<div class="mtg-header">
<h2 style="font-size:20px;font-weight:800;margin-bottom:12px;padding-bottom:10px;border-bottom:2px solid rgba(6,182,212,.3);display:flex;align-items:center;gap:8px">🕐 Совещания за период <span class="count" style="color:#0891B2">${expMeetings.length}</span></h2>

<div class="mtg-kpis">
<div class="mtg-kpi"><div class="num">${expMeetings.length}</div><div class="lbl">Всего</div></div>
<div class="mtg-kpi"><div class="num">${mtgDone.length}</div><div class="lbl">Проведено</div></div>
<div class="mtg-kpi"><div class="num">${mtgWithResults.length}</div><div class="lbl">С результатами</div></div>
<div class="mtg-kpi"><div class="num">${mtgWithoutResults.length}</div><div class="lbl">Без результатов</div></div>
</div>

${expMeetings.length>0?`
<table>
<thead><tr><th>Совещание</th><th>Дата</th><th>Время</th><th>Приоритет</th><th>Статус</th><th>Результаты</th></tr></thead>
<tbody>${expMeetings.map(m=>mtgRow(m)).join('')}</tbody>
</table>`:'<div style="text-align:center;padding:30px;color:#94A3B8">Нет совещаний за выбранный период</div>'}

${mtgWithResults.length>0?`
<div style="margin-top:24px">
<h3 style="font-size:14px;font-weight:700;color:#059669;margin-bottom:14px;display:flex;align-items:center;gap:6px">📝 Детальные результаты совещаний</h3>
${mtgWithResults.map(m=>`
<div class="result-box" style="margin-bottom:12px">
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
<div><strong style="font-size:13px">${esc(m.name)}</strong></div>
<div style="font-size:11px;color:#64748B">${fmt(m.start)} ${m.timeStart?'· '+m.timeStart:''}</div>
</div>
${m.desc?'<div style="font-size:11px;color:#64748B;margin-bottom:6px"><em>Повестка:</em> '+esc(m.desc)+'</div>':''}
<div class="res-title">Результаты:</div>
<div class="res-text">${esc(m.result)}</div>
</div>
`).join('')}
</div>`:''}
...
<div style={{position:"fixed",top:80,right:24,zIndex:2500,display:"flex",flexDirection:"column",gap:8}}>
{notifications.map(n=><NotifBanner key={n.id} notif={n} onClose={()=>removeNotification(n.id)}/>)}
</div>
...
{m.result&&<Bdg color="var(--green2)" bg="var(--green-bg)">📝 Есть результаты</Bdg>}
</div></div>
<div style={{textAlign:"right",flexShrink:0}}>
<Bdg color={ms.c}>{ms.l}</Bdg>
<div style={{fontSize:14,fontWeight:700,color:ms.c,fontFamily:"var(--mono)",marginTop:4}}>{m.progress}%</div>
{!m.result&&(m.progress>=100||(m.deadline&&m.deadline<now))&&<div style={{fontSize:10,color:"var(--amber)",marginTop:2}}>⏳ Нет результатов</div>}
</div>
</div>})}
...
<div><div style={{fontSize:22,fontWeight:800}}>📈 Аналитика задач</div><div style={{color:"var(--text3)",fontSize:13,marginTop:2}}>{anFiltered.length} задач в выборке</div></div>
...
<div className="chart-card"><div className="chart-header"><span className="chart-title">📈 Создано / Завершено (14 дней)</span></div>
<div className="bar-chart">{timeline.map((d,i)=><div key={i} className="bar-col">
<div className="bar-value">{d.completed||""}</div>
<div className="bar-fill" style={{height:`${(d.completed/maxTL)*100}%`,background:"var(--gradient2)"}}/>
<div className="bar-fill" style={{height:`${(d.created/maxTL)*100}%`,background:"var(--gradient)",opacity:.5}}/>
<div className="bar-label">{fmtS(d.day)}</div>
</div>)}</div>
<div style={{display:"flex",gap:16,justifyContent:"center",marginTop:10,fontSize:11}}>
<span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,borderRadius:3,background:"var(--gradient)"}}/>Создано</span>
<span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,borderRadius:3,background:"var(--gradient2)"}}/>Завершено</span>
</div></div>

<div className="chart-card"><div className="chart-header"><span className="chart-title">🏷️ По категориям</span></div>
{(()=>{const catData=data.categories.map(c=>({name:c,count:anFiltered.filter(t=>t.cat===c).length})).filter(c=>c.count>0);const maxC=Math.max(...catData.map(c=>c.count),1);
return catData.length?<div style={{display:"flex",flexDirection:"column",gap:10}}>
{catData.sort((a,b)=>b.count-a.count).map(c=><div key={c.name} style={{display:"flex",alignItems:"center",gap:10}}>
<span style={{width:80,fontSize:12,fontWeight:600,color:gc(c.name),textAlign:"right",flexShrink:0}}>{c.name}</span>
<div style={{flex:1,height:22,background:"#F0F4FA",borderRadius:6,overflow:"hidden"}}><div style={{height:"100%",width:`${c.count/maxC*100}%`,background:gc(c.name),borderRadius:6,transition:"width .5s"}}/></div>
<span style={{fontFamily:"var(--mono)",fontWeight:700,fontSize:13,minWidth:24}}>{c.count}</span>
</div>)}</div>:<div style={{textAlign:"center",padding:30,color:"var(--text4)"}}>Нет данных</div>})()}</div>

<div className="chart-card"><div className="chart-header"><span className="chart-title">🎯 По приоритетам</span></div>
{(()=>{const priData=data.priorities.map(p=>({name:p,count:anFiltered.filter(t=>t.pri===p).length,done:anFiltered.filter(t=>t.pri===p&&getS(t,now).l==="Выполнено").length})).filter(p=>p.count>0);const maxP=Math.max(...priData.map(p=>p.count),1);
return priData.length?<div style={{display:"flex",flexDirection:"column",gap:12}}>
{priData.map(p=><div key={p.name}>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
<span style={{fontSize:12,fontWeight:600,color:gp(p.name)}}>{p.name}</span>
<span style={{fontFamily:"var(--mono)",fontSize:11,color:"var(--text3)"}}>{p.done}/{p.count} ({p.count?Math.round(p.done/p.count*100):0}%)</span>
</div>
<div style={{height:16,background:"#F0F4FA",borderRadius:6,overflow:"hidden",position:"relative"}}>
<div style={{height:"100%",width:`${p.count/maxP*100}%`,background:gp(p.name)+"30",borderRadius:6,position:"absolute"}}/>
<div style={{height:"100%",width:`${p.done/maxP*100}%`,background:gp(p.name),borderRadius:6,position:"absolute",transition:"width .5s"}}/>
</div></div>)}</div>:<div style={{textAlign:"center",padding:30,color:"var(--text4)"}}>Нет данных</div>})()}</div>
</div>

<div style={{marginTop:20}} className="analytics-grid">
<div className="chart-card"><div className="chart-header"><span className="chart-title">🔥 Тепловая карта завершений (12 недель)</span></div>
<div style={{display:"flex",gap:3,justifyContent:"center",flexWrap:"wrap"}}>
{heatmap.map((week,wi)=><div key={wi} style={{display:"flex",flexDirection:"column",gap:3}}>
{week.map((cell,di)=><div key={di} className="heatmap-cell" style={{width:16,height:16,background:cell.count===0?"#F0F4FA":`rgba(16,185,129,${0.2+cell.count/maxHeat*0.8})`,borderRadius:3}} title={`${fmt(cell.date)}: ${cell.count} завершено`}/>)}
</div>)}</div>
<div style={{display:"flex",gap:8,justifyContent:"center",marginTop:12,fontSize:10,color:"var(--text3)",alignItems:"center"}}>
<span>Меньше</span>
{[0,.25,.5,.75,1].map(v=><div key={v} style={{width:14,height:14,borderRadius:3,background:v===0?"#F0F4FA":`rgba(16,185,129,${0.2+v*0.8})`}}/>)}
<span>Больше</span>
</div></div>

<div className="chart-card"><div className="chart-header"><span className="chart-title">📅 Ближайшие дедлайны (7 дней)</span></div>
{upcoming.length?<div style={{display:"flex",flexDirection:"column",gap:6}}>
{upcoming.slice(0,8).map(t=>{const dl=diffD(t.deadline,now);const st=getS(t,now);
return<div key={t.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px",borderRadius:10,background:dl<=1?"var(--red-bg)":dl<=3?"var(--amber-bg)":"var(--bg)",border:"1px solid var(--border)",cursor:"pointer"}} onClick={()=>setDetailTask(t)}>
<div style={{flex:1,minWidth:0}}><div style={{fontWeight:600,fontSize:13,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.name}</div>
<div style={{display:"flex",gap:4,marginTop:3}}><Bdg color={gc(t.cat)}>{t.cat}</Bdg><Bdg color={gp(t.pri)} bg={gb(t.pri)}>{t.pri}</Bdg></div></div>
<div style={{textAlign:"right",flexShrink:0}}><div style={{fontWeight:700,fontSize:12,color:dl<=1?"var(--red)":dl<=3?"var(--amber)":"var(--text2)"}}>{dl===0?"Сегодня":dl===1?"Завтра":`${dl} дн.`}</div><div style={{fontSize:11,color:st.c,fontFamily:"var(--mono)"}}>{t.progress}%</div></div>
</div>})}
</div>:<div style={{textAlign:"center",padding:30,color:"var(--text4)"}}>Нет дедлайнов на ближайшую неделю 🎉</div>}</div>
</div>

{/* Productivity score */}
{(()=>{const sc=Math.min(100,Math.round((anPct*0.4)+(onTimeRate||0)*0.3+((100-Math.min(100,(anStats.over+anStats.burn)/Math.max(1,anStats.total)*100)))*0.3));
const scoreColor=sc>=80?"var(--green)":sc>=60?"var(--amber)":"var(--red)";
const scoreLabel=sc>=80?"Отлично!":sc>=60?"Хорошо":sc>=40?"Нужно улучшить":"Критично";
return<div className="card card-glow" style={{marginTop:20,textAlign:"center",padding:30}}>
<div style={{fontSize:12,fontWeight:700,color:"var(--text3)",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>⚡ Индекс продуктивности</div>
<div style={{fontSize:56,fontWeight:800,color:scoreColor,fontFamily:"var(--mono)",lineHeight:1}}>{sc}</div>
<div style={{fontSize:14,fontWeight:700,color:scoreColor,marginTop:4}}>{scoreLabel}</div>
<div style={{marginTop:12,maxWidth:400,margin:"12px auto 0"}}><PBar value={sc} color={scoreColor} size={14}/></div>
<div style={{fontSize:11,color:"var(--text4)",marginTop:10}}>Формула: 40% завершение + 30% пунктуальность + 30% отсутствие критических</div>
</div>})()}
</div>})()}
...
{tab==="Экспорт"&&<>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:12}}>
<div><div style={{fontSize:22,fontWeight:800}}>📄 Экспорт отчёта</div><div style={{color:"var(--text3)",fontSize:13,marginTop:2}}>Формирование PDF-отчёта с фильтрами</div></div>
</div>
<div style={{display:"grid",gridTemplateColumns:"300px 1fr",gap:20}} className="two-col">
<div>
...
{data.categories.map(c=><button key={c} className="chip" style={{cursor:"pointer",background:expCats.includes(c)?gc(c)+"20":"var(--bg3)",borderColor:expCats.includes(c)?gc(c):"var(--border)",color:expCats.includes(c)?gc(c):"var(--text2)"}} onClick={()=>toggleA(expCats,setExpCats,c)}>{c}</button>)}
</div>
<div className="stitle"><span className="stitle-icon">🎯</span> Приоритеты</div>
<div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:16}}>
{data.priorities.map(p=><button key={p} className="chip" style={{cursor:"pointer",background:expPris.includes(p)?gp(p)+"20":"var(--bg3)",borderColor:expPris.includes(p)?gp(p):"var(--border)",color:expPris.includes(p)?gp(p):"var(--text2)"}} onClick={()=>toggleA(expPris,setExpPris,p)}>{p}</button>)}
</div>
<div className="stitle"><span className="stitle-icon">📊</span> Статусы</div>
<div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:16}}>
{statuses.map(s=>{const sc=s==="Выполнено"?"var(--green)":s==="В работе"?"var(--accent)":s==="Горит"?"var(--amber)":s==="Просрочено"?"var(--red)":"var(--text3)";
return<button key={s} className="chip" style={{cursor:"pointer",background:expStat.includes(s)?sc+"20":"var(--bg3)",borderColor:expStat.includes(s)?sc:"var(--border)",color:expStat.includes(s)?sc:"var(--text2)"}} onClick={()=>toggleA(expStat,setExpStat,s)}>{s}</button>})}
</div>
<div style={{padding:14,background:"var(--bg)",borderRadius:12,marginBottom:16}}>
<div style={{display:"flex",justifyContent:"space-between",fontSize:13}}><span style={{color:"var(--text3)"}}>Задач в выборке</span><span style={{fontWeight:700}}>{expFiltered.length}</span></div>
<div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginTop:6}}><span style={{color:"var(--text3)"}}>Совещаний за период</span><span style={{fontWeight:700,color:"var(--cyan2)"}}>{expMeetings.length}</span></div>
<div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginTop:6}}><span style={{color:"var(--text3)"}}>Завершено</span><span style={{fontWeight:700,color:"var(--green)"}}>{expStats.pct}%</span></div>
</div>
<button className="bp" onClick={genReport} style={{width:"100%"}}>{expGen?"⏳ Формирую...":"📊 Сформировать отчёт"}</button>
{expPreview&&<button className="bs" onClick={printReport} style={{width:"100%",marginTop:8}}>💾 Сохранить PDF</button>}
</div></div>
<div>{expPreview?<div className="card" style={{padding:0,overflow:"auto",maxHeight:"calc(100vh - 200px)"}}><iframe srcDoc={expPreview} style={{width:"100%",height:"calc(100vh - 220px)",border:"none",borderRadius:16}}/></div>:
<div className="card" style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:400,flexDirection:"column",gap:12,color:"var(--text4)"}}>
<div style={{fontSize:52}}>📄</div>
<div style={{fontSize:16,fontWeight:700}}>Настройте фильтры и нажмите «Сформировать отчёт»</div>
<div style={{fontSize:13}}>Отчёт будет включать задачи и совещания</div>
</div>}</div>
</div></>}
...
{/* ═══ MODALS ═══ */}
{showForm&&<TForm onClose={()=>{setShowForm(false);setEditTask(null)}} initial={editTask&&editTask.type!=="meeting"?editTask:null} onSubmit={t=>{if(editTask&&editTask.type!=="meeting"){updT(editTask.id,t);showToast("Задача обновлена ✅")}else{addT({...t,type:"task"});showToast("Задача добавлена ✨")}}} cats={data.categories} pris={data.priorities}/>}
{showMForm&&<MForm onClose={()=>{setShowMForm(false);setEditTask(null)}} initial={editTask&&editTask.type==="meeting"?editTask:null} onSubmit={m=>{if(editTask&&editTask.type==="meeting"){updT(editTask.id,m);showToast("Совещание обновлено ✅")}else{addT({...m,type:"meeting"});showToast("Совещание добавлено 🕐")}}} cats={data.categories} pris={data.priorities}/>}
{showResultForm&&<ResultForm task={showResultForm} onClose={()=>setShowResultForm(null)} onSave={(id,patch)=>{saveResult(id,patch);showToast(patch.progress>=100?"Совещание завершено ✅":"Результаты сохранены 📝")}}/>}
{detailTask&&<TDetail task={detailTask} now={now} onClose={()=>setDetailTask(null)} onEdit={t=>{setDetailTask(null);setEditTask(t);if(t.type==="meeting")setShowMForm(true);else setShowForm(true)}} onDelete={id=>{delT(id);showToast("Удалено 🗑","error")}} onProg={setProg} onResult={t=>{setDetailTask(null);setShowResultForm(t)}}/>}
...
# ═══════════════════════════════════════════════════════
# PYTHON BACKEND — webview API
# ═══════════════════════════════════════════════════════

import json, os, uuid, datetime, threading, tempfile, webview

DATA_FILE = os.path.join(os.path.expanduser("~"), ".smartplanner_data.json")

DEFAULT_DATA = {
    "categories": ["Рабочая", "Проект", "Обучение", "Личная", "Рутина"],
    "priorities": ["Высокий", "Средний", "Низкий"],
    "protectedCats": ["Рабочая", "Проект", "Обучение", "Личная", "Рутина"],
    "protectedPris": ["Высокий", "Средний", "Низкий"],
    "tasks": []
}
...
class Api:
    def get_data(self):
        return json.dumps(load_data(), ensure_ascii=False)
    ...
    def add_task(self, task_json):
        ...
        if task.get("progress", 0) >= 100 and not task.get("completedAt"):
    ...
    def update_task(self, task_id, patch_json):
        ...
        for t in data["tasks"]:
            if t.get("id") == task_id:
                t.update(patch)
                if t.get("progress", 0) >= 100 and not t.get("completedAt"):
                    t["completedAt"] = datetime.date.today().isoformat()
                elif t.get("progress", 0) < 100:
                    t["completedAt"] = None
                break
        save_data(data)
    ...
    def delete_category(self, name):
        data = load_data()
        if name in data.get("protectedCats", []):
            return
    ...
    def add_priority(self, name):
        data = load_data()
        if name not in data["priorities"]:
            data["priorities"].append(name)
            save_data(data)

    def delete_priority(self, name):
        data = load_data()
        if name in data.get("protectedPris", []):
            return
        data["priorities"] = [p for p in data["priorities"] if p != name]
        save_data(data)

    def save_pdf_report(self, html_content):
        try:
            # Save HTML to temp file then open in browser for printing
            ...
            return json.dumps({"success": True, "path": tmp.name})
